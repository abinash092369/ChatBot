...
export class UserService {
  ...
  public async getUserAnalytics(userId: string) {
    const [totalConversations, totalToolExecutions, totalKnowledgeBases, totalPrompts, totalWorkflows, usageLogs] = await Promise.all([
      prisma.conversation.count({ where: { userId, deletedAt: null } }),
      prisma.toolExecution.count({ where: { conversation: { userId } } }),
      prisma.knowledgeBase.count({ where: { userId } }),
      prisma.promptTemplate.count({ where: { userId } }),
      prisma.workflow.count({ where: { userId } }),
      prisma.usageLog.findMany({ where: { userId }, take: 20, orderBy: { createdAt: 'desc' } }),
    ]);
